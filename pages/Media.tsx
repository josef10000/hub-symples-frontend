...
const Media: React.FC = () => {
  ...
  // 🔹 LISTAR MÍDIAS (CORREÇÃO PRINCIPAL)
  useEffect(() => {
    mediaService
      .getAll()
      .then((res) => {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const res = await mediaService.upload(e.target.files[0]);
      if (res?.data) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-400 mt-1">
            Áudios e imagens usados em fluxos e respostas do bot.
          </p>
        </div>
      ...
      {items.length === 0 ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
          {items.map((item) => (
            <div
              key={item.id}
              className="group relative bg-slate-900 rounded-lg border border-slate-800 overflow-hidden aspect-square flex flex-col hover:border-slate-600 transition-colors"
            >
              {/* PREVIEW */}
      ...
      >
        <p>
          Este arquivo pode estar sendo usado em fluxos ativos. A exclusão é
          permanente.
        </p>
      </Modal>
    </div>
