...
const AI: React.FC = () => {
  ...
  const loadBotData = async (botId: string) => {
    try {
      const cfg = await aiService.getConfig(botId);
      const kb = await aiService.getKnowledge(botId);
      const ex = await aiService.getExamples(botId);
      setConfig(cfg);
  ...
  if (!config) return <div className="text-white p-8">Carregando Módulo de IA...</div>;
